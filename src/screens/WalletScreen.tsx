...
export default function WalletScreen() {
  ...
  const fetchBalances = async () => {
    try {
      ...
      const totalNow = enrichedBalances.reduce(
        (acc, curr) => acc + (curr.usdtValue || 0),
        0
      );

      const totalYesterday = await calculatePortfolioValue(
        db,
        balanceRows,
        yesterday
      );
